...
const infoIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="theme-icon theme-empty__icon" data-size="xl"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>`;

function renderErrorBu({ htmlId, withSecondary = false }) {
  const titleId = `${htmlId}-title`;
  const primaryHtml = `<a class="theme-button" data-variant="primary" data-size="md" href="https://browsehappy.com/"><span class="theme-button__label">${t('theme.error.browser-unsupported.primary-cta')}</span></a>`;
  const secondaryHtml = withSecondary
    ? `<a class="theme-button" data-variant="ghost" data-size="md" href="/"><span class="theme-button__label">${t('theme.error.browser-unsupported.secondary-cta')}</span></a>`
    : '';
  return `
    <main class="theme-error-shell theme-error-browser-unsupported" data-variant="default" id="${htmlId}">
      <section class="theme-error-shell__panel">
        <div class="theme-empty" data-size="lg" role="region" aria-labelledby="${titleId}">
          <div class="theme-empty__illustration" aria-hidden="true">${infoIconSvg}</div>
          <div class="theme-empty__content">
            <h1 id="${titleId}" class="theme-empty__title">${t('theme.error.browser-unsupported.title')}</h1>
            <div class="theme-empty__body"><p>${t('theme.error.browser-unsupported.body')}</p></div>
...
const bodyHtml = `
  <section class="error-bu-story" data-testid="error-bu-root">
    ...
    <section class="error-bu-story__section" aria-labelledby="error-bu-section-default">
      <h2 id="error-bu-section-default" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.browser-unsupported.story.section.default')}</h2>
      <p class="error-bu-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.browser-unsupported.story.explainer.default')}</p>
    ...
    <section class="error-bu-story__section" aria-labelledby="error-bu-section-with-continue">
      <h2 id="error-bu-section-with-continue" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.browser-unsupported.story.section.with-continue')}</h2>
      <p class="error-bu-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.browser-unsupported.story.explainer.with-continue')}</p>
...
.error-bu-story__frame .theme-error-shell { min-block-size: 100%; }
