...
const heroSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 160" width="200" height="160" aria-hidden="true" class="theme-error-offline__hero"><defs><linearGradient id="s-eo-grad" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="var(--color-bg-sunken)"/><stop offset="1" stop-color="var(--color-border-subtle)"/></linearGradient></defs><rect x="20" y="30" rx="14" ry="14" width="160" height="100" fill="url(#s-eo-grad)" stroke="var(--color-border-default)" stroke-width="2"/><g transform="translate(100 95)" stroke="var(--color-text-tertiary)" stroke-width="3" stroke-linecap="round" fill="none"><path d="M -30 -10 Q 0 -36 30 -10" opacity="0.45"/><path d="M -20 -2 Q 0 -22 20 -2" opacity="0.65"/><path d="M -10 6 Q 0 -6 10 6" opacity="0.85"/><circle cx="0" cy="14" r="3" fill="var(--color-text-tertiary)"/><line x1="-32" y1="-18" x2="32" y2="22" stroke="var(--color-danger)" stroke-width="4"/></g></svg>`;

function renderErrorOffline({ htmlId, withCached = false }) {
  const titleId = `${htmlId}-title`;
  const secondaryHtml = withCached
    ? `<a class="theme-button" data-variant="ghost" data-size="md" href="#cached"><span class="theme-button__label">${t('theme.error.offline.secondary-cta')}</span></a>`
    : '';
  return `
    <main class="theme-error-shell theme-error-shell--hero theme-error-offline" data-variant="hero" id="${htmlId}">
      <section class="theme-error-shell__panel">
        <div class="theme-empty" data-size="lg" role="region" aria-labelledby="${titleId}">
          <div class="theme-empty__illustration" aria-hidden="true">${heroSvg}</div>
          <div class="theme-empty__content">
            <h1 id="${titleId}" class="theme-empty__title">${t('theme.error.offline.title')}</h1>
            <div class="theme-empty__body"><p>${t('theme.error.offline.body')}</p></div>
            <div class="theme-empty__actions">
              <a class="theme-button" data-variant="primary" data-size="md" href="javascript:location.reload();"><span class="theme-button__label">${t('theme.error.offline.primary-cta')}</span></a>
              ${secondaryHtml}
            </div>
          </div>
        </div>
...
const bodyHtml = `
  <section class="error-offline-story" data-testid="error-offline-root">
    ...
    <section class="error-offline-story__section" aria-labelledby="error-offline-section-default">
      <h2 id="error-offline-section-default" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.offline.story.section.default')}</h2>
      <p class="error-offline-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.offline.story.explainer.default')}</p>
    ...
    <section class="error-offline-story__section" aria-labelledby="error-offline-section-cached">
      <h2 id="error-offline-section-cached" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.offline.story.section.cached')}</h2>
      <p class="error-offline-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.offline.story.explainer.cached')}</p>
...
<template>
  <Story title="Patterns/Error-Offline" group="Ring 3" :layout="{ type: 'single' }">
...
.error-offline-story__frame .theme-error-shell { min-block-size: 100%; }
