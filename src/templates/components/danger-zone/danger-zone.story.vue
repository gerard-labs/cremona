...
function renderInlineDangerZone({ id, expected = 'SUPPRIMER', preFilled = '' }) {
  ...
  return `
    <article class="theme-card theme-danger-zone"
      ...
      <header class="theme-card__header theme-danger-zone__header">
        <h3 class="theme-typography theme-danger-zone__title" data-variant="h3">${t('theme.danger-zone.story.sample.title')}</h3>
      </header>
      <div class="theme-card__body theme-danger-zone__body">
        <p class="theme-typography" data-variant="body" data-color="secondary">${t('theme.danger-zone.story.sample.description')}</p>
        <div class="theme-danger-zone__confirmation">
          <label for="${id}-input" class="theme-label">${t('theme.danger-zone.input.label', { expected })}</label>
          <input type="text"
                 id="${id}-input"
                 class="theme-input theme-danger-zone__input"
                 value="${preFilled}"
                 placeholder="${t('theme.danger-zone.input.placeholder')}"
                 autocomplete="off" spellcheck="false"
                 aria-describedby="${id}-help">
          <p id="${id}-help" class="theme-danger-zone__help theme-typography" data-variant="caption" data-color="tertiary">${t('theme.danger-zone.input.help', { expected })}</p>
        </div>
      </div>
      <footer class="theme-card__footer theme-danger-zone__footer">
        <button type="button"
                class="theme-button theme-danger-zone__cta"
                data-variant="destructive" data-size="md"
                ${armed ? '' : 'disabled aria-disabled="true"'}>
          <span class="theme-button__label">${t('theme.danger-zone.story.sample.cta')}</span>
        </button>
      </footer>
    </article>`;
}
...
function renderModalDangerZone({ id }) {
  return `
    <article class="theme-card theme-danger-zone"
      ...
      <div class="theme-card__body theme-danger-zone__body">
        <p class="theme-typography" data-variant="body" data-color="secondary">${t('theme.danger-zone.story.sample.description-modal')}</p>
      </div>
      <footer class="theme-card__footer theme-danger-zone__footer">
        <button type="button"
                class="theme-button theme-danger-zone__cta"
...
const bodyHtml = `
  <section class="danger-zone-story" aria-labelledby="danger-zone-story-title">
    ...
    <section class="danger-zone-story__section">
      <h2 class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.danger-zone.story.section.inline-default')}</h2>
      <p class="danger-zone-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.danger-zone.story.explainer.inline-default')}</p>
      <div class="danger-zone-story__frame">${renderInlineDangerZone({ id: 'story-inline-default', preFilled: '' })}</div>
    </section>

    <section class="danger-zone-story__section">
      <h2 class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.danger-zone.story.section.inline-armed')}</h2>
      <p class="danger-zone-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.danger-zone.story.explainer.inline-armed')}</p>
      <div class="danger-zone-story__frame">${renderInlineDangerZone({ id: 'story-inline-armed', preFilled: 'SUPPRIMER' })}</div>
    </section>

    <section class="danger-zone-story__section">
      <h2 class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.danger-zone.story.section.modal')}</h2>
      <p class="danger-zone-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.danger-zone.story.explainer.modal')}</p>
