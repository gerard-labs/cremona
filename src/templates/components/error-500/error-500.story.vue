...
const alertCircleIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="cremona-icon cremona-empty__icon" data-size="xl"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>`;

function renderError500({ htmlId, withSecondary = true }) {
  const titleId = `${htmlId}-title`;
  const primaryHtml = `<a class="cremona-button" data-variant="primary" data-size="md" href="javascript:location.reload();"><span class="cremona-button__label">${t('theme.error.500.primary-cta')}</span></a>`;
  const secondaryHtml = withSecondary
    ? `<a class="cremona-button" data-variant="ghost" data-size="md" href="/"><span class="cremona-button__label">${t('theme.error.500.secondary-cta')}</span></a>`
    : '';
  return `
    <main class="cremona-error-shell cremona-error-500" data-variant="default" id="${htmlId}">
      <section class="cremona-error-shell__panel">
        <div class="cremona-empty" data-size="lg" role="region" aria-labelledby="${titleId}">
          <div class="cremona-empty__illustration" aria-hidden="true">${alertCircleIconSvg}</div>
          <div class="cremona-empty__content">
            <h1 id="${titleId}" class="cremona-empty__title">${t('theme.error.500.title')}</h1>
            <div class="cremona-empty__body"><p>${t('theme.error.500.body')}</p></div>
...
const bodyHtml = `
  <section class="error-500-story" data-testid="error-500-root">
    ...
    <section class="error-500-story__section" aria-labelledby="error-500-section-default">
      <h2 id="error-500-section-default" class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.error.500.story.section.default')}</h2>
      <p class="error-500-story__explainer cremona-typography" data-variant="caption" data-color="tertiary">${t('theme.error.500.story.explainer.default')}</p>
    ...
    <section class="error-500-story__section" aria-labelledby="error-500-section-primary-only">
      <h2 id="error-500-section-primary-only" class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.error.500.story.section.primary-only')}</h2>
      <p class="error-500-story__explainer cremona-typography" data-variant="caption" data-color="tertiary">${t('theme.error.500.story.explainer.primary-only')}</p>
...
<template>
  <Story title="Errors/500" group="Ring 3" :layout="{ type: 'single' }">
...
.error-500-story__frame .cremona-error-shell { min-block-size: 100%; }
