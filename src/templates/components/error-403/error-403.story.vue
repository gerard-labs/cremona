...
const eyeIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="cremona-icon cremona-empty__icon" data-size="xl"><path d="M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0"/><circle cx="12" cy="12" r="3"/></svg>`;

function renderError403({ htmlId, withSecondary = false }) {
  const titleId = `${htmlId}-title`;
  const primaryHtml = `<a class="cremona-button" data-variant="primary" data-size="md" href="/"><span class="cremona-button__label">${t('theme.error.403.primary-cta')}</span></a>`;
  const secondaryHtml = withSecondary
    ? `<a class="cremona-button" data-variant="ghost" data-size="md" href="mailto:admin@example.com"><span class="cremona-button__label">${t('theme.error.403.secondary-cta')}</span></a>`
    : '';
  return `
    <main class="cremona-error-shell cremona-error-403" data-variant="default" id="${htmlId}">
      <section class="cremona-error-shell__panel">
        <div class="cremona-empty" data-size="lg" role="region" aria-labelledby="${titleId}">
          <div class="cremona-empty__illustration" aria-hidden="true">${eyeIconSvg}</div>
          <div class="cremona-empty__content">
            <h1 id="${titleId}" class="cremona-empty__title">${t('theme.error.403.title')}</h1>
            <div class="cremona-empty__body"><p>${t('theme.error.403.body')}</p></div>
...
const bodyHtml = `
  <section class="error-403-story" data-testid="error-403-root">
    ...
    <section class="error-403-story__section" aria-labelledby="error-403-section-default">
      <h2 id="error-403-section-default" class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.error.403.story.section.default')}</h2>
      <p class="error-403-story__explainer cremona-typography" data-variant="caption" data-color="tertiary">${t('theme.error.403.story.explainer.default')}</p>
    ...
    <section class="error-403-story__section" aria-labelledby="error-403-section-request-access">
      <h2 id="error-403-section-request-access" class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.error.403.story.section.request-access')}</h2>
      <p class="error-403-story__explainer cremona-typography" data-variant="caption" data-color="tertiary">${t('theme.error.403.story.explainer.request-access')}</p>
...
<template>
  <Story title="Errors/403" group="Ring 3" :layout="{ type: 'single' }">
...
.error-403-story__frame .cremona-error-shell { min-block-size: 100%; }
