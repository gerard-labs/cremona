...
function renderError503({ htmlId, withSecondary = false }) {
  ...
  const primaryHtml = `<a class="theme-button" data-variant="primary" data-size="md" href="javascript:location.reload();"><span class="theme-button__label">${t('theme.error.503.primary-cta')}</span></a>`;
  const secondaryHtml = withSecondary
    ? `<a class="theme-button" data-variant="ghost" data-size="md" href="/status"><span class="theme-button__label">${t('theme.error.503.secondary-cta')}</span></a>`
    : '';
  return `
    <main class="theme-error-shell theme-error-503" data-variant="default" id="${htmlId}">
      <section class="theme-error-shell__panel">
        <div class="theme-empty" data-size="lg" role="region" aria-labelledby="${titleId}">
          ...
          <div class="theme-empty__content">
            <h1 id="${titleId}" class="theme-empty__title">${t('theme.error.503.title')}</h1>
            <div class="theme-empty__body"><p>${t('theme.error.503.body')}</p></div>
...
const bodyHtml = `
  <section class="error-503-story" data-testid="error-503-root">
    ...
    <section class="error-503-story__section" aria-labelledby="error-503-section-default">
      <h2 id="error-503-section-default" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.503.story.section.default')}</h2>
      <p class="error-503-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.503.story.explainer.default')}</p>
    ...
    <section class="error-503-story__section" aria-labelledby="error-503-section-with-status">
      <h2 id="error-503-section-with-status" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.503.story.section.with-status')}</h2>
      <p class="error-503-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.503.story.explainer.with-status')}</p>
...
.error-503-story__frame .theme-error-shell { min-block-size: 100%; }
