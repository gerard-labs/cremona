...
function renderDatePickerStub({ id, name, value, placeholder, min, max }) {
  ...
  return `
    <div class="cremona-popover cremona-date-picker" data-controller="popover date-picker" data-popover-placement-value="bottom-start" data-popover-offset-value="4" data-date-picker-value-value="${value || ''}"${min ? ` data-date-picker-min-value="${min}"` : ''}${max ? ` data-date-picker-max-value="${max}"` : ''} data-date-picker-locale-value="fr">
      <div class="cremona-date-picker__trigger">
        <input type="text" class="cremona-input cremona-date-picker__input" id="${id}-input" data-popover-target="trigger" data-date-picker-target="input" role="combobox" aria-haspopup="dialog" aria-expanded="false" readonly placeholder="${placeholder}" value="${value || ''}" />
      </div>
...
function renderFormDateRange({ id, label, startValue = '', endValue = '', invalid = false, help, min, max }) {
  return `
    <div class="cremona-field cremona-form-date-range${invalid ? ' cremona-field--invalid' : ''}">
      <fieldset class="cremona-form-date-range__fieldset" aria-labelledby="${id}-legend">
        <legend id="${id}-legend" class="cremona-label cremona-form-date-range__legend">${label}</legend>
        <div class="cremona-form-date-range__wrap" data-controller="form-date-range" data-form-date-range-start-date-value="${startValue}" data-form-date-range-end-date-value="${endValue}">
          <div class="cremona-form-date-range__field" data-form-date-range-target="startInput">
            <label class="cremona-form-date-range__label" for="${id}-start-input">${t('theme.form.date-range.label.start')}</label>
          ...
          <div class="cremona-form-date-range__field" data-form-date-range-target="endInput">
            <label class="cremona-form-date-range__label" for="${id}-end-input">${t('theme.form.date-range.label.end')}</label>
          ...
          <p class="cremona-form-date-range__error" data-form-date-range-target="errorMessage" role="alert"${invalid ? '' : ' hidden'}>${t('theme.form.date-range.error.start-after-end')}</p>
        </div>
      </fieldset>
      ${help ? `<p class="cremona-field__help">${help}</p>` : ''}
    </div>
  `;
}
...
const bodyHtml = `
  <section class="form-date-range-story" aria-labelledby="form-date-range-story-title">
    ...
    <section class="form-date-range-story__section">
      <h2 class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.form.date-range.story.section.default')}</h2>
    ...
    <section class="form-date-range-story__section">
      <h2 class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.form.date-range.story.section.with-values')}</h2>
    ...
    <section class="form-date-range-story__section">
      <h2 class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.form.date-range.story.section.invalid-range')}</h2>
    ...
    <section class="form-date-range-story__section">
      <h2 class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.form.date-range.story.section.with-min-max')}</h2>
...
<template>
  <Story title="Forms/Date Range" group="Ring 3" :layout="{ type: 'single' }">
