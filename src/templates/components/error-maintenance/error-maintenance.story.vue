...
function renderErrorMaintenance({ htmlId, withEta = false }) {
  ...
  const secondaryHtml = withEta
    ? `<a class="theme-button" data-variant="ghost" data-size="md" href="#notify"><span class="theme-button__label">${t('theme.error.maintenance.secondary-cta')}</span></a>`
    : '';
  return `
    <main class="theme-error-shell theme-error-shell--hero theme-error-maintenance" data-variant="hero" id="${htmlId}">
      <section class="theme-error-shell__panel">
        <div class="theme-empty" data-size="lg" role="region" aria-labelledby="${titleId}">
          ...
          <div class="theme-empty__content">
            <h1 id="${titleId}" class="theme-empty__title">${t('theme.error.maintenance.title')}</h1>
            <div class="theme-empty__body"><p>${bodyText}</p></div>
            <div class="theme-empty__actions">
              <a class="theme-button" data-variant="primary" data-size="md" href="/status"><span class="theme-button__label">${t('theme.error.maintenance.primary-cta')}</span></a>
              ${secondaryHtml}
            </div>
          </div>
        </div>
...
const bodyHtml = `
  <section class="error-maintenance-story" data-testid="error-maintenance-root">
    ...
    <section class="error-maintenance-story__section" aria-labelledby="error-maintenance-section-default">
      <h2 id="error-maintenance-section-default" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.maintenance.story.section.default')}</h2>
      <p class="error-maintenance-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.maintenance.story.explainer.default')}</p>
    ...
    <section class="error-maintenance-story__section" aria-labelledby="error-maintenance-section-with-eta">
      <h2 id="error-maintenance-section-with-eta" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.error.maintenance.story.section.with-eta')}</h2>
      <p class="error-maintenance-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.error.maintenance.story.explainer.with-eta')}</p>
...
<template>
  <Story title="Patterns/Error-Maintenance" group="Ring 3" :layout="{ type: 'single' }">
...
.error-maintenance-story__frame .theme-error-shell { min-block-size: 100%; }
