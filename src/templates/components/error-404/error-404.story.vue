...
const searchIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="cremona-icon cremona-empty__icon" data-size="xl"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>`;

function renderError404({ htmlId, withSecondary = false }) {
  const titleId = `${htmlId}-title`;
  const primaryHtml = `<a class="cremona-button" data-variant="primary" data-size="md" href="/"><span class="cremona-button__label">${t('theme.error.404.primary-cta')}</span></a>`;
  const secondaryHtml = withSecondary
    ? `<a class="cremona-button" data-variant="ghost" data-size="md" href="javascript:history.back();"><span class="cremona-button__label">${t('theme.error.404.secondary-cta')}</span></a>`
    : '';
  return `
    <main class="cremona-error-shell cremona-error-404" data-variant="default" id="${htmlId}">
      <section class="cremona-error-shell__panel">
        <div class="cremona-empty" data-size="lg" role="region" aria-labelledby="${titleId}">
          <div class="cremona-empty__illustration" aria-hidden="true">${searchIconSvg}</div>
          <div class="cremona-empty__content">
            <h1 id="${titleId}" class="cremona-empty__title">${t('theme.error.404.title')}</h1>
            <div class="cremona-empty__body"><p>${t('theme.error.404.body')}</p></div>
...
const bodyHtml = `
  <section class="error-404-story" data-testid="error-404-root">
    ...
    <section class="error-404-story__section" aria-labelledby="error-404-section-default">
      <h2 id="error-404-section-default" class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.error.404.story.section.default')}</h2>
      <p class="error-404-story__explainer cremona-typography" data-variant="caption" data-color="tertiary">${t('theme.error.404.story.explainer.default')}</p>
    ...
    <section class="error-404-story__section" aria-labelledby="error-404-section-with-back">
      <h2 id="error-404-section-with-back" class="cremona-typography" data-variant="overline" data-color="tertiary">${t('theme.error.404.story.section.with-back')}</h2>
      <p class="error-404-story__explainer cremona-typography" data-variant="caption" data-color="tertiary">${t('theme.error.404.story.explainer.with-back')}</p>
...
.error-404-story__frame .cremona-error-shell { min-block-size: 100%; }
