...
function triggerButton(label, ariaLabel, popoverId, opts = {}) {
  ...
  const trailingHtml = trailing ? `<span class="theme-button__icon theme-button__icon--trailing">${icon(trailing, 'sm')}</span>` : '';
  return `
    <button type="button" class="theme-button" data-variant="${variant}" data-size="${size}"
      data-popover-target="trigger"
      aria-haspopup="dialog" aria-expanded="false" aria-controls="${popoverId}" ${ariaAttr}>
      ${leadingHtml}<span class="theme-button__label">${label}</span>${trailingHtml}
    </button>
  `;
}
...
function renderPopover({ placement = 'bottom', triggerHtml, contentHtml, offset = 8 }) {
  ...
  return `
    <div class="theme-popover"
      ...
      ${triggerHtml(id)}
      <div id="${id}" class="theme-popover__content"
...
const bodyHtml = `
  <section class="popover-story" data-testid="popover-root">
    ...
    <section class="popover-story__section" aria-labelledby="pp-section-default">
      <h2 id="pp-section-default" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.popover.story.section.default')}</h2>
      <p class="popover-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.popover.story.explainer.default')}</p>
      <div class="popover-story__row">
        ${renderPopover({
          placement: 'bottom',
          triggerHtml: (id) => triggerButton(S.tagsTrigger, S.ariaTags, id),
          contentHtml: `
            <div class="theme-popover__header">
              <h3 class="theme-popover__title">${S.tagsTitle}</h3>
            </div>
            <p>${S.tagsBody}</p>
          `,
        })}
    ...
    <section class="popover-story__section" aria-labelledby="pp-section-placements">
      <h2 id="pp-section-placements" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.popover.story.section.placements')}</h2>
      <p class="popover-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.popover.story.explainer.placements')}</p>
    ...
    <section class="popover-story__section" aria-labelledby="pp-section-with-form">
      <h2 id="pp-section-with-form" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.popover.story.section.with-form')}</h2>
      <p class="popover-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.popover.story.explainer.with-form')}</p>
      <div class="popover-story__row">
        ${renderPopover({
          placement: 'bottom',
          ...
          contentHtml: `
            <div class="theme-popover__header">
              <h3 class="theme-popover__title">${S.shareTitle}</h3>
            </div>
            <div class="popover-story__share-form">
              <label class="theme-typography" data-variant="caption" data-color="tertiary">${S.shareLinkLabel}</label>
              <input type="text" class="theme-input" value="${S.shareLinkValue}" readonly />
              <button type="button" class="theme-button" data-variant="primary" data-size="sm">
                <span class="theme-button__icon theme-button__icon--leading">${icon('copy', 'sm')}</span>
                <span class="theme-button__label">${S.shareCopy}</span>
              </button>
            </div>
          `,
        })}
    ...
    <section class="popover-story__section" aria-labelledby="pp-section-collision">
      <h2 id="pp-section-collision" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.popover.story.section.collision')}</h2>
      <p class="popover-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.popover.story.explainer.collision')}</p>
    ...
    <section class="popover-story__section" aria-labelledby="pp-section-long">
      <h2 id="pp-section-long" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.popover.story.section.long-content')}</h2>
      <p class="popover-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.popover.story.explainer.long-content')}</p>
      <div class="popover-story__row">
        ${renderPopover({
          placement: 'bottom',
    ...
    <section class="popover-story__section" aria-labelledby="pp-section-events">
      <h2 id="pp-section-events" class="theme-typography" data-variant="overline" data-color="tertiary">${t('theme.popover.story.section.events')}</h2>
      <p class="popover-story__explainer theme-typography" data-variant="caption" data-color="tertiary">${t('theme.popover.story.explainer.events')}</p>
      <div class="popover-story__events">
        <div class="popover-story__events-trigger-row">
          ${renderPopover({
            ...
            contentHtml: `
              <div class="theme-popover__header">
                <h3 class="theme-popover__title">${S.eventsTitle}</h3>
              </div>
              <p>${S.eventsBody}</p>
            `,
          })}
...
<template>
  <Story title="Compounds/Popover" group="Ring 2" :layout="{ type: 'single' }">
...
.popover-story__share-form .theme-input { font: var(--typography-body); }
