 ...
 *
 * Validates:
 *   - connect() stamps the input target onto the first .theme-input
...
describe('InputGroupController', () => {
  ...
  it('connect() stamps data-input-group-target="input" on the first .theme-input', async () => {
    await mount(`
      <div class="theme-input-group" data-controller="input-group" data-action="click->input-group#delegateClick">
        <span class="theme-input-group__prefix">€</span>
        <input class="theme-input" type="number">
      </div>
    `);
    const input = document.querySelector('input.theme-input');
  ...
  it('focusInput() focuses the inner input', async () => {
    await mount(`
      <div class="theme-input-group" data-controller="input-group" data-action="click->input-group#delegateClick">
        ...
        <span class="theme-input-group__suffix">EUR</span>
      </div>
    `);
    const input = document.querySelector('input.theme-input');
    const focusSpy = vi.spyOn(input, 'focus');
    // Click on suffix → delegates to focusInput
    document.querySelector('.theme-input-group__suffix').click();
  ...
  it('clear() wipes value, dispatches input + change, refocuses', async () => {
    await mount(`
      <div class="theme-input-group" data-controller="input-group" data-action="click->input-group#delegateClick">
        <input class="theme-input" type="text" value="hello">
        <span class="theme-input-group__suffix">
  ...
  it('delegateClick on a non-button prefix focuses the input', async () => {
    ...
    const input = document.querySelector('input.theme-input');
    const focusSpy = vi.spyOn(input, 'focus');
    document.querySelector('.theme-input-group__prefix').click();
    expect(focusSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it('delegateClick on a button INSIDE the prefix does NOT steal focus from the button', async () => {
    await mount(`
      <div class="theme-input-group" data-controller="input-group" data-action="click->input-group#delegateClick">
        <span class="theme-input-group__prefix">
    ...
    const input = document.querySelector('input.theme-input');
    const focusSpy = vi.spyOn(input, 'focus');
    document.querySelector('.theme-input-group__prefix button').click();
